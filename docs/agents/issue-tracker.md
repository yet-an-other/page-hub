...
- **Resolve**: `gh issue comment <n> --body "<answer>"`, then `gh issue close <n>`, then append a context pointer to the map's Decisions-so-far. Put durable handoffs in a tracked Markdown file and link that file. Use a gist only for sanitized evidence or probe output that does not belong in the repository. Use `web-share` when the handoff needs a rendered public artifact.
