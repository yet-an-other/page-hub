...
```text
                         +-----------------------+
operator browser ------> | DNS, TLS, and router  | ------> authentication ------> Page Hub
...
| Single-user authentication | Admit only the configured operator to the manager and management API. This may be an authentication gateway or authentication implemented by Page Hub. The concrete mechanism remains a deployment decision. |
...
1. authentication callbacks and other routes required by the selected authentication mechanism;
2. the exact manager page, manager assets, and management API routes;
...
- Unauthenticated requests to manager pages and APIs receive an authentication response. They never fall through to a Publication.
...
- State-changing management requests reach only Page Hub. Clients do not write accepted Publication state by bypassing Page Hub.
...
Authentication may run at the router or inside Page Hub. Either arrangement must enforce the same boundary:

- every manager page and management API request requires the configured operator identity;
...
- object-store credentials remain on a trusted server and are never sent to browser code; and
- signing out invalidates the Page Hub session and, where supported, the identity-provider session.

The choice between an authentication gateway and application-managed authentication belongs to the runtime and authentication decision. The deployment contract does not require either one.
...
- upload-size and storage-quota limits; and
...
5. Configure authentication and the ordered route rules on a non-public or temporary origin.
6. Verify manager isolation and public Publication behavior.
7. Point public DNS at the tested TLS entry point.
8. Adopt existing Publications without changing their objects or URLs.
9. Move mutation clients to Page Hub only after the management path is healthy.
...
- the authentication provider, operator admission rule, and recovery procedure;
